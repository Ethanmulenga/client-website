...
function loadUsers() {
  db.ref("users").on("value", (snapshot) => {
    ...
    snapshot.forEach((childSnapshot) => {
      ...
      card.innerHTML = `
        <div>
          <h4 class="font-bold">${user.name || 'User'}</h4>
          <p class="text-sm text-gray-600">Email: ${user.email}</p>
          <p class="text-sm">Role: ${user.role || 'customer'}</p>
        </div>
        <button onclick="deleteUser('${key}')" class="bg-red-500 text-white px-3 py-1 rounded">Delete</button>
      `;
